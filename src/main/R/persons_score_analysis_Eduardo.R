...
output_legs_base_case_a <- "../../../../outputs-1pct/output-Base_Case/berlin-v6.4-1pct/berlin-v6.4.output_legs.csv.gz"
...
base_case_a_car_km %>% summarise(total_car_km = sum(person_car_km))
...
output_legs_siba_a <- "../../../../outputs-1pct/output-SiBa/berlin-v6.4-1pct/berlin-v6.4.output_legs.csv.gz"
...
siba_a_car_km %>% summarise(total_car_km = sum(person_car_km))
...
siba_a_base_case_a_car_km_diff_out <- siba_a_base_case_a_car_km_diff %>%
    filter(car_km_diff < -60 | car_km_diff > 60) %>%
    select(person,car_km_diff) %>%
    # inner_join(bind_rows(
    #   "Base Case" = base_case_a_car_km,
    #   "SiBa" = siba_a_car_km,
    #   .id = "source"), by = "person")
    summarise(total_car_diff_out = sum(car_km_diff))
...
base_case_a_car_km_new_pt_users %>% summarise(total_car_km_new_pt_users = sum(person_car_km))
