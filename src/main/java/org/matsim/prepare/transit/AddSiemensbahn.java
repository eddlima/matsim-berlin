...
//		var stop8_w_e=scheduleFactory.createTransitRouteStop(stop1_facility_w_e,13,14);
		...
		//route
		//S-Bahn
		var route_e_w = scheduleFactory.createTransitRoute(Id.create("SiBa_v2_e_w", TransitRoute.class),
		...
		var route_w_e = scheduleFactory.createTransitRoute(Id.create("SiBa_v2_w_e", TransitRoute.class),
		...
		for (int i = 3 * 3600; i < 24 * 3600; i += 600) {
			...
			var vehicle = scenario.getTransitVehicles().getFactory().createVehicle(Id.createVehicleId("SiBa_v2_vehicle_e_w_" + "100" + i), vehicleTypeSBahn);
		...
		for (int i = 3 * 3600; i < 24 * 3600; i += 600) {
			var departure = scheduleFactory.createDeparture(Id.create("departure_" + i, Departure.class), i);
			var vehicle = scenario.getTransitVehicles().getFactory().createVehicle(Id.createVehicleId("SiBa_v2_vehicle_w_e_" + "100" + i), vehicleTypeSBahn);
			departure.setVehicleId(vehicle.getId());
		...
		new NetworkWriter(network).write(root.resolve("berlin-v6.4-network-SiBa_v2+Bus-10min.xml.gz").toString());
		new TransitScheduleWriter(scenario.getTransitSchedule()).writeFile(root.resolve("berlin-v6.4-transitSchedule-SiBa_v2+Bus-10min.xml.gz").toString());
		new MatsimVehicleWriter(scenario.getTransitVehicles()).writeFile(root.resolve("berlin-v6.4-transitVehicles-SiBa_v2+Bus-10min.xml.gz").toString());
	}
